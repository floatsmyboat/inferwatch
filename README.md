...
| Setting | Default | Accepts | Environment variable | Notes |
|---|---|---|---|---|
| `retention.raw_days` | `7.0` | 0.5–3650 | `INFERWATCH_RETENTION_RAW_DAYS` | Per-request detail older than this is deleted. The rollups below outlive it, so long-range charts survive the cutoff. |
| `retention.sample_days` | `30.0` | 0.5–3650 | `INFERWATCH_RETENTION_SAMPLE_DAYS` | GPU samples, engine samples and the event log are trimmed to this. This is where the bulk of the database is: these tables are the largest by a wide margin, so this setting moves the total size far more than the rollup limits do. |
| `retention.rollup_1m_days` | `0.0` | 0–3650 | `INFERWATCH_RETENTION_ROLLUP_1M_DAYS` | 0 keeps them forever. One row per minute per model per class that saw traffic, so the cost scales with traffic and with how many models are in use, not with the calendar. This is the only table with no natural ceiling; bounding it is what stops the database growing without limit. |
| `retention.rollup_1h_days` | `0.0` | 0–3650 | `INFERWATCH_RETENTION_ROLLUP_1H_DAYS` | 0 keeps them forever, which is usually right: they are 36x coarser than the 1-minute rollups and cost very little, and they are what lets a chart reach past every other cutoff here. |
...
- `rollup_1m` and `rollup_1h`: **kept indefinitely by default**
  (`retention.rollup_1m_days`, `retention.rollup_1h_days`; `0` means forever).

### What that costs, and where the size actually is

The rollups are the only tables with no natural ceiling — everything else is
bounded by a window above, so it plateaus. But they are also small. Measured on
the development host over 29 days of real traffic:

| | rows/day | on disk | growth |
|---|---:|---:|---|
| `rollup_1m` | 1,138 | 7.2 MB | ~90 MB/year |
| `rollup_1h` | 32 | 0.2 MB | ~3 MB/year |

A minute rollup is one row per model per class that saw traffic, at ~217 bytes
(most of it the two serialised histograms), so the cost follows traffic and the
number of distinct models rather than the calendar — a mostly idle host writes
roughly one row per busy minute, a saturated one across several models writes
several.

That is not where a large database comes from. On the same host the sample
tables settle at roughly ten times the rollups' size — `vllm_samples` 113 MB,
`image_samples` 103 MB, `gpu_samples` 96 MB, `vllm_hist` 96 MB at a 30-day
window — for about **470 MB total**, reached once every table has filled its
window. So `retention.sample_days` is the setting that moves the total; the
rollup limits are what stop it rising forever.

Capping `rollup_1h` is rarely worth it: at 3 MB/year it is the cheapest data in
the schema, and it is what lets a chart reach past every other cutoff.
